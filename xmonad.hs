...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- crop the screen
    , ((modm, xK_i), spawn "flameshot gui")

    -- print the entire screen    
    , ((modm .|. shiftMask, xK_i), spawn "flameshot full -p ~/Images")

    -- launch emacs
    , ((modm, xK_semicolon), spawn "emacsclient -c")

    -- launch xcolor, show the picked color and copy it to clipboard
    , ((modm .|. controlMask, xK_i), spawn "xcolor | xargs notify-send \"Color\"")

    -- lauch scratchpad with telegram
    , ((modm .|. shiftMask, xK_t), namedScratchpadAction myScratchpads "telegram")
    ...
    , ((modm .|. shiftMask, xK_s), toggleWindowInAllWorkspaces)
...
myScratchpads = [
  ...
  , NS "telegram" "telegram-desktop" getTelegram resizeTelegram
  ]
  where
    getSpotifyTerm = title =? "spotify"
    getTelegram = className =? "TelegramDesktop"
    resizeTerm = customFloating $ W.RationalRect 0.01 0.01 0.98 0.5
    resizeTelegram = customFloating $ W.RationalRect 0.70 0.0 0.30 1.0
...
myStartupHook :: X ()
myStartupHook = do
  spawnOnce "feh --bg-fill /home/consoli/Images/space.jpg &"
  spawnOnce "redshift &"
...
main :: IO ()
main = do
  myBar <- spawnPipe "xmobar /home/consoli/.config/xmobar/xmobarrc"
  xmonad $ ewmh $ docks $ defaults myBar
